...
public class Code03_FindWordInMatrix {
     ...
     * 不能走重复路
     * 从m[i][j]这个字符出发，能不能找到str[k...]这个后缀串
     * 该方法不能改动态规划。因为可变参数不只是整形，还有m这个二维数组
     *
     * @since 2022-03-09 08:28:37
     */
    public static boolean noLoop(char[][] m, int i, int j, char[] str, int k) {
        ...
        // 走过的路，将其ASCII码标为0，后面再走到这个位置就不会匹配上了
        m[i][j] = 0;
        boolean ans = false;
        ...
        // 走完之后，将字符改回去.
        // 采用的是深度优先。恢复现场是为了防止某个不通的后路，将字符改过之后，它又没走通。
        // 导致上游再去尝试其他通路的时候，受到其影响
        // 深度优先，依次退回的时候，要恢复现场
        m[i][j] = str[k];
