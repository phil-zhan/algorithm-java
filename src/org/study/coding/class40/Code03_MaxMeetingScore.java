 ...
 * 准备一个时间指针。初始值是0
 * 去遍历所有会议
 * 来到当前会议。看看能不能召开，能召开，就将其放到小跟堆里面去。同时将时间指针往后移动10【每个会议的持续时间是10】
 *
 * 去到下一个会议。如果不能召开。说明有前面的会议与当前会议冲突了。
 * 看看堆顶元素的收益与当前收益哪个大。
 * 如果堆顶元素的大，直接扔掉当前会议
 * 如果当前会议的收益大，就将当前会议替换掉堆顶元素
 *
 * 堆顶的元素，是当前能安排的会议中，收益最小的一个
 *
 *
 * @since 2022-06-13 23:47:11
 */
