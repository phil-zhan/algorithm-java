...
public class Code04_RegularExpressionMatch {
    ...
    public static boolean process(char[] s, char[] e, int si, int ei) {
        ...
        // 前面也可能 ei 和 ei+1 的位置搞定完了一段相同的串。但是这个过程红，后面的都没搞定。
        // 就会来到相同串的下一个位置。也就是那个与前面字符不相同的位置。此时就会从while里面跳出来。但是此时 si 已经加加过了。也就是 si 在那个和前面字符不一样的位置
        // 此时就相当于整个相同串都由 ei 和 ei+1 搞定
